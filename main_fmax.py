...
fmax_uav_list = [1e9, 1.2e9, 1.4e9, 1.6e9, 1.8e9, 2e9]  # fmax từ 1GHz đến 2GHz
...
for fmax_uav in fmax_uav_list:  # số UAV cố định
    print(f"Đang chạy với fmax UAV {fmax_uav}Hz...")

    # Khởi tạo env với số UE cụ thể
    env = UAVMECEnv(num_ues=100, fmax_uav=fmax_uav)
    ...
    print("Bắt đầu MARL...")
    marl = MARL(num_ues=100, num_uavs=10, num_episodes=25000, fmax_uav=fmax_uav)
    ...
    print("Bắt đầu MAFRL...")
    mafrl = MAFRL(num_ues=100, num_uavs=10, num_episodes=25000, fmax_uav=fmax_uav)
    ...
    mafrl_ddqn = MAFRL_DDQN(
        num_ues=100, num_uavs=10, num_episodes=25000, fmax_uav=fmax_uav
    )
...
plt.plot(fmax_uav_list, greedy_results, marker="^", label="Greedy Offload", linewidth=2)
plt.plot(fmax_uav_list, random_results, marker="d", label="Random Execute", linewidth=2)
plt.plot(fmax_uav_list, local_results, marker="s", label="Local Execute", linewidth=2)
plt.plot(fmax_uav_list, marl_results, marker="o", label="MARL", linewidth=2)
plt.plot(fmax_uav_list, mafrl_results, marker="x", label="MAFRL", linewidth=2)
plt.plot(fmax_uav_list, mafrl_ddqn_results, marker="v", label="MAFRL-DDQN", linewidth=2)
