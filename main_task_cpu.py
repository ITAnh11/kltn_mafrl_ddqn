...
task_cpu_cycles_min_list = [1e6, 20e6, 40e6, 60e6, 80e6]  # cycles
task_cpu_cycles_max_list = [60e6, 80e6, 100e6, 120e6, 140e6]  # cycles
task_cpu_cycles_size_list = [
    (task_cpu_cycles_min_list[i] + task_cpu_cycles_max_list[i]) / 2
    for i in range(len(task_cpu_cycles_min_list))
]
...
for i in range(len(task_cpu_cycles_size_list)):  # số UAV cố định
    print(f"=== Case {i+1}/{len(task_cpu_cycles_min_list)} ===")

    # Khởi tạo env với số UE cụ thể
    env = UAVMECEnv(
        num_ues=100,
        task_cpu_cycles_min=task_cpu_cycles_min_list[i],
        task_cpu_cycles_max=task_cpu_cycles_max_list[i],
    )
...
plt.plot(task_cpu_cycles_size_list, marl_results, marker="o", label="MARL", linewidth=2)
plt.plot(
    task_cpu_cycles_size_list, mafrl_results, marker="x", label="MAFRL", linewidth=2
)
plt.plot(
    task_cpu_cycles_size_list,
    mafrl_ddqn_results,
...
plt.xlabel("Task CPU Size (bits)", fontsize=13)
plt.ylabel("Sum Energy Consumption (J)", fontsize=13)
plt.title("Impact of Task CPU Size on Sum Energy Consumption", fontsize=14)
plt.legend(fontsize=12)
plt.grid(True)
plt.tight_layout()
plt.savefig("all_vs_task_cpu.png")
