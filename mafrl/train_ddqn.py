...
outdir = f"results/results_train_mafrl_ddqn_ue_{num_ue}_{timestamp}"
os.makedirs(outdir, exist_ok=True)


mafrl_ddqn = MAFRL_DDQN(num_ues=num_ue, num_uavs=10, num_episodes=40000, outdir=outdir)
time_start = datetime.now()
mafrl_ddqn.train()
mafrl_ddqn.save_model(f"mafrl/model/mafrl_ddqn_model_ue_{num_ue}_{timestamp}.pth")
...
with open(os.path.join(outdir, "training_time_mafrl_ddqn.txt"), "w") as f:
